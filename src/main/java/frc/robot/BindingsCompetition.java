...
public final class BindingsCompetition {
    ...
    static void OperatorBindings(HID_Xbox_Subsystem dc) {
        ...
        // AutoShootm 
        ManualShoot.negate().and(operator.rightBumper()).onTrue(new AutoShooting(ShootingTarget.Speaker, 45.0, 3000.0));
        ...
        ClimberCalibrate.and(operator.povUp()).whileTrue(new ClimbVel(Climber.ClimbCalibrateVel));
        ClimberCalibrate.and(operator.povDown().whileTrue(new ClimbVel(-Climber.ClimbCalibrateVel)));
