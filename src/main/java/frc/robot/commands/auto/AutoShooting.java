...
public class AutoShooting extends SequentialCommandGroup {
  ...
  /** Face to and shooter sequence just for temporary */
  public AutoShooting(ShootingTarget target, double rpm, double angle) {
    drivetrain = RobotContainer.getSubsystem(SwerveDrivetrain.class);
    limelight = RobotContainer.getSubsystem(Limelight_Subsystem.class);
    ...
    if (target == ShootingTarget.Speaker) {
      ...
      addCommands(new ShooterServoSequence(angle, 800.0));
    } else {
      // Trap
      addCommands(new ShooterServoSequence(angle, 1000.0));
    }
  }
