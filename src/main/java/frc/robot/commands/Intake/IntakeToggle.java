...
public class IntakeToggle extends Command implements BlinkyLightUser {
  ...
  public void end(boolean interrupted) {
    // if driver releases button go back to safe position, or if we have the note safely in our possession, go to safe position
  ...
  public boolean isFinished() {
    ...
    return false;
    //return (count >= DONE_COUNT); done when we hit the count - if we want to go back automatically
  }
}
